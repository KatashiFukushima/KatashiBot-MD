...
function ensureProtoCompat(root) {
  const seen = new Set()
  const visit = (node) => {
    if (!node || (typeof node !== 'object' && typeof node !== 'function') || seen.has(node)) return
    seen.add(node)

    if (typeof node.create === 'function' && typeof node.fromObject !== 'function') {
      node.fromObject = (value) => node.create(value || {})
    }
...
ensureProtoCompat(proto)
if (WAProto) ensureProtoCompat(WAProto)

if (!proto.WebMessageInfo && WAProto?.WebMessageInfo) {
  proto.WebMessageInfo = WAProto.WebMessageInfo
}
...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    const parseMentionInline = (text = '') => {
      if (typeof text !== 'string' || !text) return []
      const matches = text.match(/@([0-9]{5,16}|0)/g)
      return matches ? matches.map(v => v.slice(1) + '@s.whatsapp.net') : []
    }

    const buildPlainTextFromContent = (content = {}) => {
      const body = String(content?.text || content?.caption || content?.contentText || '')
      const footer = String(content?.footer || '')
      const extras = []

      const legacyButtons = Array.isArray(content?.buttons) ? content.buttons : []
      if (legacyButtons.length) {
        extras.push('Opciones:')
        legacyButtons.forEach((btn, i) => {
          const label = btn?.buttonText?.displayText || btn?.displayText || btn?.buttonId || btn?.id || `Opcion ${i + 1}`
          extras.push(`${i + 1}. ${label}`)
        })
      }

      const templateButtons = Array.isArray(content?.templateButtons) ? content.templateButtons : []
      if (templateButtons.length) {
        extras.push('Acciones:')
        templateButtons.forEach((btn, i) => {
          const label = btn?.quickReplyButton?.displayText || btn?.urlButton?.displayText || btn?.callButton?.displayText || `Accion ${i + 1}`
          extras.push(`${i + 1}. ${label}`)
        })
      }
      ...
      if (sections.length) {
        ...
        sections.forEach((section) => {
          if (section?.title) extras.push(`${section.title}`)
          const rows = Array.isArray(section?.rows) ? section.rows : []
          rows.forEach((row, i) => {
            const title = row?.title || row?.id || `Elemento ${i + 1}`
            const desc = row?.description ? ` - ${row.description}` : ''
            extras.push(`${i + 1}. ${title}${desc}`)
          })
        })
      }

      return [body, footer, extras.join('\n')].filter(Boolean).join('\n').trim() || '(sin contenido)'
    }

    const shouldForcePlainText = (content = {}) => {
      return Boolean(
        Array.isArray(content?.buttons) ||
        Array.isArray(content?.templateButtons) ||
        (Array.isArray(content?.sections) && typeof content?.buttonText === 'string') ||
    ...
    conn.sendMessage = async (jid, content = {}, sendOptions = {}) => {
      if (!shouldForcePlainText(content)) {
        return originalSendMessage(jid, content, sendOptions)
      }

      const plainText = buildPlainTextFromContent(content)
      const mentions = content?.mentions || content?.contextInfo?.mentionedJid || parseMentionInline(plainText)
      return originalSendMessage(jid, { text: plainText, mentions }, sendOptions)
    }
    ...
    conn.relayMessage = async (jid, message = {}, relayOptions = {}) => {
      ...
      if (!interactive && !template && !list) {
        return originalRelayMessage(jid, message, relayOptions)
      }

      const textFromInteractive = interactive
        ? [interactive?.body?.text, interactive?.footer?.text].filter(Boolean).join('\n')
        : ''
      const textFromTemplate = template
        ? [template?.hydratedContentText, template?.hydratedFooterText].filter(Boolean).join('\n')
        : ''
      const textFromList = list
        ? [list?.title, list?.description, list?.footerText].filter(Boolean).join('\n')
        : ''

      const plainText = [textFromInteractive, textFromTemplate, textFromList].filter(Boolean).join('\n').trim() || '(mensaje interactivo oculto)'
      return originalSendMessage(jid, { text: plainText }, {})
    }
...
const safeQuoted = quoted && typeof quoted === 'object' && quoted.key && typeof quoted.key === 'object' ? quoted : undefined
let canalId = ["120363178367665133@newsletter", "120363178367665133@newsletter", "120363178367665133@newsletter", "120363178367665133@newsletter", "120363178367665133@newsletter", "120363178367665133@newsletter"]
let canalNombre = ["💫👁️‍🗨️ 【Katashi Fukushima】 🥷🏻🚀", "💫👁️‍🗨️ 【Katashi Fukushima】 🥷🏻🚀", "💫👁️‍🗨️ 【Katashi Fukushima】 🥷🏻🚀", "💫👁️‍🗨️ 【Katashi Fukushima】 🥷🏻🚀", "💫👁️‍🗨️ 【Katashi Fukushima】 🥷🏻🚀", "💫👁️‍🗨️ 【Katashi Fukushima】 🥷🏻🚀"]
...
const contextInfo = {
mentionedJid: await conn.parseMention(normalizedText),
isForwarded: true,
forwardingScore: 1, 
...
const messageOptions = { ...safeOptions, text: normalizedText, contextInfo }
delete messageOptions.quoted
const sendOptions = safeQuoted ? { ...safeOptions, quoted: safeQuoted } : { ...safeOptions }
return conn.sendMessage(jid, messageOptions, sendOptions)
}}
},
...
getName: {
  value(jid = "", withoutContact = false) {
    ...
    if (!jid || typeof jid !== 'string') return withoutContact ? '' : 'Desconocido'
    ...
    let userName = global.db.data.users[numericId]?.name
    if (!userName) {
      userName = jid.endsWith('@s.whatsapp.net') || jid.endsWith('@lid')
        ? PhoneNumber('+' + numericId).getNumber('international')
        : jid
    }
...
export function smsg(conn, m, hasParent) {
    ...
    if (!m.mediaMessage) delete m.download
     ...
     if (m.msg?.contextInfo?.mentionedJid?.length) {
      ...
      m.msg.contextInfo.mentionedJid = mentioned.map(id => {
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        mentionedJid: {
            get() {
            const ctxMentions = this.msg?.contextInfo?.mentionedJid
            if (Array.isArray(ctxMentions) && ctxMentions.length) return ctxMentions

            // Fallback: algunos clientes no envian mentionedJid y solo dejan @numero en texto.
            const raw = String(this.text || '')
            const parsed = [...raw.matchAll(/@([0-9]{5,20})/g)].map((v) => `${v[1]}@s.whatsapp.net`)
