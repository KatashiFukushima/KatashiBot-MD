...
if (!mentions[0] && !m.quoted) return m.reply(texto, m.chat, {mentions: conn.parseMention(texto)})
if (mentions.some(jid => selfIds.has(normalize(jid)))) return
const user = mentions[0] ? mentions[0] : m.quoted?.sender
if (!user) return m.reply(texto, m.chat, {mentions: conn.parseMention(texto)})
