...
let _name = await conn.getName(senderJid) || 'Anónimo'
const senderNumber = senderJid.replace(/@.+$/, '')
const senderIntl = PhoneNumber('+' + senderNumber).getNumber('international')
let sender = senderIntl === undefined ? '' : senderIntl + (_name == senderIntl ? '' : ' ~' + _name)
let chat = await conn.getName(chatJid)
let img
try {
if (global.opts['img'])
...
name_user = m.messageStubParameters.map((participant) => {
...
console.log(m.messageStubParameters.map((participant) => {
const normalized = normalizeStubParticipant(participant)
