...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    const toLegacyButtonNativeFlow = ({ buttons = [], templateButtons = [] }) => {
      const result = []

      if (Array.isArray(buttons)) {
        for (const btn of buttons) {
          const label = btn?.buttonText?.displayText || btn?.displayText || ''
          const id = btn?.buttonId || btn?.id || ''
          if (!label || !id) continue
          result.push({
            name: 'quick_reply',
            buttonParamsJson: JSON.stringify({ display_text: String(label), id: String(id) })
          })
        }
      }

      if (Array.isArray(templateButtons)) {
        for (const btn of templateButtons) {
          if (btn?.quickReplyButton) {
            result.push({
              name: 'quick_reply',
              buttonParamsJson: JSON.stringify({
                display_text: String(btn.quickReplyButton.displayText || ''),
                id: String(btn.quickReplyButton.id || '')
              })
            })
            continue
          }
          if (btn?.urlButton) {
            const url = String(btn.urlButton.url || '')
            if (!url) continue
            result.push({
              name: 'cta_url',
              buttonParamsJson: JSON.stringify({
                display_text: String(btn.urlButton.displayText || 'URL'),
                url,
                merchant_url: url
              })
            })
          ...
          if (btn?.callButton) {
            result.push({
              name: 'cta_call',
              buttonParamsJson: JSON.stringify({
                display_text: String(btn.callButton.displayText || 'Call'),
                id: String(btn.callButton.phoneNumber || '')
              })
            })
          }
        }
    ...
    conn.sendMessage = async (jid, content = {}, sendOptions = {}) => {
      const hasLegacyButtons = Array.isArray(content?.buttons) || Array.isArray(content?.templateButtons)
      const hasLegacyList = Array.isArray(content?.sections) && typeof content?.buttonText === 'string'

      if (!hasLegacyButtons && !hasLegacyList) {
        return originalSendMessage(jid, content, sendOptions)
      }

      const bodyText = String(content?.text || content?.caption || '')
      const footerText = String(content?.footer || '')
      const mentioned = content?.mentions || content?.contextInfo?.mentionedJid || parseMentionInline(`${bodyText} ${footerText}`)

      const buttons = hasLegacyButtons
        ? toLegacyButtonNativeFlow({ buttons: content?.buttons, templateButtons: content?.templateButtons })
        : [{
          name: 'single_select',
          buttonParamsJson: JSON.stringify({
            title: String(content?.buttonText || 'Opciones'),
            sections: Array.isArray(content?.sections) ? content.sections : []
          })
        }]

      if (!buttons.length) {
        return originalSendMessage(jid, { ...content, text: bodyText || '(sin contenido)' }, sendOptions)
      }

      const interactiveMessage = proto.Message.InteractiveMessage.create({
        body: proto.Message.InteractiveMessage.Body.create({ text: bodyText || '(sin contenido)' }),
